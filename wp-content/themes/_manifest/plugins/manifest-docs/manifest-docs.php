...
class Docs {
	...
	private function __construct() {
		...
		add_action( 'init', array( $this, 'load_doc_scripts' ) );
		add_action( 'wp_ajax_doc_order', array( $this, 'doc_order' ) );
		add_action( 'wp_ajax_nopriv_doc_order', array( $this, 'doc_order' ) );
	...
	function create_doc_taxonomy() {
		...
		register_taxonomy( 'doc_tags', 'docs', $args );
	...
	function no_index_no_follow() {
		...
		if ( is_post_type_archive( 'docs' ) || 'docs' == get_post_type() ) {
	...
	function load_doc_scripts() {
		...
		wp_enqueue_script(
			'doc_ajax',
			get_template_directory_uri() . '/plugins/manifest-docs/assets/js/doc_ajax.js',
			array( 'jquery' ),
			'1',
			true
		);
		...
		wp_localize_script(
			...
			array(
				...
				'nonce' => wp_create_nonce( 'doc_ajax_nonce' )
			)
		);
	...
	function doc_order() {
		...
		if ( wp_verify_nonce( $nonce, 'doc_ajax_nonce' ) ) {
			...
			if ( $tax_query ) {
				...
				$args = array_merge( array( 'doc_tags' => $value ), $args );
	...
	function create_placeholder_doc() {
		...
		//
		if ( 0 == count( get_posts( array( 'post_type' => 'docs', 'posts_per_page' => 1) ) ) ) {
